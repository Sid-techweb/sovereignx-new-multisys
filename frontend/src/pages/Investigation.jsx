...
import { Send, RefreshCw, AlertTriangle, CheckCircle, Sparkles, ChevronRight, FileText, Activity, History, Plus } from 'lucide-react';
...
export default function Investigation({ modelConfig, isConnected }) {
  ...
  const [activeConversationId, setActiveConversationId] = useState(() => {
    return localStorage.getItem('active_investigation_convo_id') || null;
  });
  ...
  const fetchConversations = async () => {
    try {
      const res = await fetch(`${API_BASE}/investigations/conversations`, {
      ...
      console.error("Failed to load investigation conversations", err);
    }
  };

  const loadConversationHistory = async (convoId) => {
    try {
      const res = await fetch(`${API_BASE}/investigations/conversations/${convoId}`, {
        headers: { 'X-API-Key': API_KEY }
      });
      if (res.ok) {
        const data = await res.json();
        ...
        localStorage.setItem('active_investigation_convo_id', convoId);
      } else {
        localStorage.removeItem('active_investigation_convo_id');
  ...
  const handleAnalyze = async (e) => {
    ...
    try {
      ...
      if (!convoId) {
        const createRes = await fetch(`${API_BASE}/investigations/conversations`, {
          method: 'POST',
          headers: {
        ...
        if (!createRes.ok) throw new Error('Failed to create investigation conversation.');
        const createData = await createRes.json();
        convoId = createData.conversation_id;
        setActiveConversationId(convoId);
        localStorage.setItem('active_investigation_convo_id', convoId);
      }

      const response = await fetch(`${API_BASE}/investigations/conversations/${convoId}/messages`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-API-Key': API_KEY
  ...
  const handleCreateCase = async (msgItem, isEscalated = false) => {
    if (!msgItem) return;
    ...
    try {
      const res = await fetch(`${API_BASE}/cases`, {
        ...
        body: JSON.stringify({
          query: msgItem.query,
          answer: msgItem.answer,
          confidence: msgItem.confidence,
          requires_human_review: isEscalated || msgItem.requires_human_review,
          escalation_reason: msgItem.escalation_reason,
          retrieved_chunks: msgItem.retrieved_chunks || [],
          tool_executions: msgItem.tool_executions || []
        })
      });
  ...
  return (
    <div className="space-y-6">
        ...
        actions={
          <div className="flex items-center gap-2.5">
            <button
              ...
              className="px-3.5 py-1.5 bg-white/[.05] hover:bg-white/[.1] text-console-text text-xs font-mono font-semibold rounded border border-console-line transition-all flex items-center gap-1.5"
            >
              <History className="w-3.5 h-3.5 text-console-amber" />
            ...
            <button
              ...
              className="px-3.5 py-1.5 bg-white/[.05] hover:bg-white/[.1] text-console-text text-xs font-mono font-semibold rounded border border-console-line transition-all flex items-center gap-1.5"
            >
              <Plus className="w-3.5 h-3.5 text-console-amber" />
              <span>New conversation</span>
            </button>
            <a
              href="/reports"
              className="px-3.5 py-1.5 bg-console-amber hover:brightness-105 text-[#0b1620] text-xs font-mono font-bold rounded shadow transition-all"
            >
              Generate report
            </a>
          </div>
      ...
      {isHistoryOpen && (
        <div className="bg-console-panel border border-console-line rounded-lg p-4 font-mono space-y-3">
          <div className="flex items-center justify-between pb-2 border-b border-console-lineSoft">
            <div className="flex items-center gap-2">
              <History className="w-4 h-4 text-console-amber" />
              <h3 className="text-xs font-bold text-console-text uppercase tracking-widest">
                PAST INVESTIGATIONS ({conversations.length})
              </h3>
            </div>
            <button
              type="button"
          ...
          <div className="max-h-60 overflow-y-auto space-y-2 pr-1">
            {conversations.length === 0 ? (
              <p className="text-xs text-console-muted italic">No past investigations recorded.</p>
            ) : (
              conversations.map((c) => (
                <button
                  ...
                  className={`w-full text-left p-2.5 rounded border transition-all flex items-center justify-between ${
                    c.conversation_id === activeConversationId
                      ? 'bg-console-amberSoft border-console-amber/50 text-console-amber'
                      : 'bg-console-inset border-console-line text-console-text2 hover:bg-white/[.05] hover:text-console-text'
                  }`}
                >
                  <div className="truncate text-xs font-sans font-medium pr-2">
                    {c.title}
                  </div>
                  <div className="text-[10px] text-console-muted shrink-0 font-mono">
                    ...
                    <div key={item.id || idx} className="space-y-4">
                      ...
                      <div className="flex gap-3 items-start bg-console-panelSolid/80 border border-console-line rounded-lg p-5 font-sans space-y-4">
                        ...
                        <div className="flex-1 space-y-4">
                          ...
                          <div className="flex items-center justify-between border-b border-console-lineSoft pb-2.5 font-mono text-[10px]">
                            ...
                            <div className="flex items-center gap-2">
                              <span className={`px-2.5 py-0.5 rounded border font-mono font-bold tabular-nums text-[10px] uppercase tracking-wider ${
                                ...
                                {(item.confidence * 100).toFixed(1)}% GROUNDED
                              </span>
                            </div>
                          </div>
                          ...
                          {/* Cited Evidence Chips (Deduplicated at display layer) */}
                          {item.retrieved_chunks && item.retrieved_chunks.length > 0 && (() => {
                            const seen = new Set();
                            const uniqueDisplayNames = [];
                            for (const chunk of item.retrieved_chunks) {
                              const name = chunk.filename || 'Document';
                              if (!seen.has(name)) {
                                seen.add(name);
                                uniqueDisplayNames.push(name);
                              }
                            }
                            return (
                              <div className="pt-3 border-t border-console-lineSoft space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {uniqueDisplayNames.map((name, cIdx) => (
                                    <span 
                                      key={cIdx} 
                            ...
                            <div className="bg-console-amberSoft border border-console-amber/40 rounded p-3.5 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 text-console-amber font-mono text-xs">
                              <div className="flex items-start gap-2.5">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-console-text2 mt-0.5 font-sans">{item.escalation_reason}</p>
                                </div>
                              </div>
                              <button
                                type="button"
                                onClick={() => handleCreateCase(item, true)}
                          ...
                          <ReasoningTrailDisclosure 
                            toolExecutions={item.tool_executions} 
