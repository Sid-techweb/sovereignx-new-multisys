...
class TestToolRegistryAllowlisting(unittest.TestCase):
    ...
    def test_existing_tools_still_registered(self):
        """Preserve all existing tools -- adding new ones must not remove any."""
        from app.services.tools import LocalToolRegistry
        registry = LocalToolRegistry()
        names = {t.name for t in registry.list_tools()}
        for expected in (
...
class TestSandboxExecution(unittest.TestCase):
    ...
    def test_filesystem_isolation_cannot_read_host_outside_workspace(self):
        """The sandbox's only writable/readable mount is its own throwaway
        workspace -- it cannot see files created by a DIFFERENT sandbox run
        (each is a fresh temp dir), proving isolation rather than a shared
        host filesystem view."""
        from app.services.sandbox import execute_python_sandboxed
        first = execute_python_sandboxed(
            "with open('marker.txt', 'w') as f:\n    f.write('from run one')\nprint('wrote marker')"
        )
        self.assertIn("wrote marker", first["stdout"])
        second = execute_python_sandboxed(
            "import os\nprint('marker present' if os.path.exists('marker.txt') else 'marker absent')"
        )
        self.assertIn("marker absent", second["stdout"])
