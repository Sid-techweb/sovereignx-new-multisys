...
class TestDocumentIntakeAndExtraction(unittest.TestCase):
    ...
    def test_image_extraction_ocr_and_captioning(self):
        """Test Image: Image processing performs OCR/captioning using Qwen2-VL"""
        try:
            import qwen_vl_utils
            import torch
        except ImportError:
            self.skipTest("qwen_vl_utils or torch not installed in this environment")

        with patch("transformers.Qwen2VLForConditionalGeneration.from_pretrained") as mock_model_from_pretrained, \
             patch("transformers.AutoProcessor.from_pretrained") as mock_processor_from_pretrained, \
             patch("qwen_vl_utils.process_vision_info") as mock_process_vision:
