"""
Bounded multi-step agent loop -- the project statement's core "agentic AI
workbench" requirement: plan multi-step tasks, call local tools, observe
results, and produce a final deliverable, rather than a single question-in/
answer-out turn.
...
Deliberately a CONTROLLED loop, not an open-ended autonomous agent:
  ...
  - Every tool call goes through the existing LocalToolRegistry, which only
    ever executes registered tools (see test_agent_tools.py's allowlisting
    tests) -- the model cannot invoke anything outside that allowlist.
