"""
Sandboxed local code execution -- the MRPL/SIH26117 problem statement's
explicit "a coding task run and verified in a sandbox" requirement.

Isolation is Docker-based, not a bare subprocess: subprocess-only isolation
cannot enforce real network/memory/CPU limits on Windows (no cgroups, no
`resource` module), so Docker gives the actual guarantees this module
promises ("network blocked", "CPU/memory constraints") rather than a
best-effort approximation of them. Docker Desktop is already part of this
stack (see docker-compose.yml's Postgres/pgvector service).
...
Runtime allowlist: Python only, deliberately -- keeps the surface area
small (adding another language means auditing another base image) and
matches this project's current tooling (Ollama/Qwen for text, Python for
deterministic verification, both already the app's languages).
"""
import logging
import subprocess
import tempfile
...
def _docker_available() -> bool:
    try:
        result = subprocess.run(["docker", "info"], capture_output=True, timeout=5.0)
