...
class BGEM3EmbeddingProvider(EmbeddingProvider):
    ...
    def get_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        logger.info(f"Splitting {total_chunks} chunks into {total_batches} IPC batches (batch_size={batch_size})")
        
        for idx, i in enumerate(range(0, total_chunks, batch_size), 1):
            batch = texts[i:i + batch_size]
            t0 = time.perf_counter()
            batch_embeddings = manager.embed(batch)
            dt = time.perf_counter() - t0
            logger.info(f"IPC Batch {idx}/{total_batches} ({len(batch)} chunks) completed in {dt:.3f}s")
